...
############################################################
# NEW: Helper — ensure text does NOT overlap components
############################################################

def bbox_overlap(b1, b2):
    """Check if two bounding boxes overlap."""
    return not (b1["x2"] < b2["x1"] or 
                b1["x1"] > b2["x2"] or 
                b1["y2"] < b2["y1"] or 
                b1["y1"] > b2["y2"])


def sample_non_overlapping_text_position(component_bboxes, 
                                         grid_size, 
                                         text_w=0.6, 
                                         text_h=0.3, 
                                         max_retry=80):
    """
    Randomly place text inside schematic canvas WITHOUT touching any component.
    text_w, text_h 在 circuitikz 的坐标中表示文字占的宽高（可调）
    """
    for _ in range(max_retry):
        ...
        ok = True
        for cb in component_bboxes:
            if bbox_overlap(cb, text_bbox):
                ok = False
                break
    ...
    # 如果 max_retry 次都不成功，随便放，但你可以选择不放
    return None, None
...
if __name__ == "__main__":
    ...
    component_bboxes = [] #添加component box
    for dev in devices:
...
"""
    ...
    #     )
    ############################################################
    # NEW: Generate text labels that DO NOT overlap components
    ############################################################
    text_labels = [
        "VDD","Vout","Vin","node1","Ibias","GND","Vref","CLK","out","bias",
    ]
    ...
    for _ in range(num_labels):
        ...
        # 使用新函数，找到不会覆盖组件的坐标
        lx, ly = sample_non_overlapping_text_position(
            component_bboxes,
            grid_size,
            text_w=0.6,
            text_h=0.3
        ...
        rot = random.choice([-20, -10, 0, 10, 20])
        color = random.choice(["gray", "blue!40", "red!60", "black"])
        font_size = random.choice(["\\tiny", "\\scriptsize", "\\footnotesize", "\\small"])
...
#         nodeType = "ground"
#     elif device_type == "Voltage":
#         nodeType = "american voltage source"
#     elif device_type == "Current":
#         nodeType = "american current source"
#     else:
#         nodeType = "ERROR"
...
#                 if dev["position"] == device["position"]:
#                     device["position"] = (
#                         random.randint(0, device["grid_size"] - 1),
#                         random.randint(0, device["grid_size"] - 1),
#                     )
#                     flag = True
#                 if (
...
#         counters[dev_type] += 1
#         dev = create_device(dev_type, counters[dev_type], grid_size)
#         # 给 device 也带上 grid_size，方便 assign_nets_to_device 里重新采样位置
#         dev["grid_size"] = grid_size
#         dev_with_nets = assign_nets_to_device(dev, nets, devices)
#         if dev_with_nets is not None:
#             devices.append(dev_with_nets)
#         else:
...
# def is_far_from_components(lx, ly, comp_positions, min_dist=1.8):
#     """
#     粗略判定 (lx, ly) 是否离每个元件中心足够远。
#     使用的是欧式距离，单位和 dev_x/dev_y 相同（grid 坐标系）。
#     """
...
#     # 记录所有元件中心位置，用来做“文字不重叠组件”的约束
#     component_positions = []

#     for dev in devices:
#         dev_x, dev_y = dev["position"]
#         component_positions.append((dev_x, dev_y))
...
#         "bias",
#     ]
#     num_labels = random.randint(3, 8)  # 文本数量

#     for _ in range(num_labels):
#         # 尝试多次寻找一个不靠近任何组件的位置
#         for _try in range(50):
#             lx = random.uniform(0, grid_size)          # X：全局范围
#             ly = random.uniform(0, grid_size)          # Y：全局范围（和组件一致）
#             if is_far_from_components(lx, ly, component_positions, min_dist=1.8):
#                 # 找到一个离所有组件都够远的位置
#                 label = random.choice(text_labels)
#                 rot = random.choice([-20, -10, 0, 10, 20])
#                 color = random.choice(["gray", "blue!40", "red!60", "black"])
...
#                     f"\\node[{color}, rotate={rot}] at ({lx:.2f},{ly:.2f}) {{{font_size} {label}}};"
#                 )
#                 break  # 成功放一个就退出当前 label 的 retry 循环
